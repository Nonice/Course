...
' Service.Ціна, '+
' Service.Обсяг, '+
' Service.Можливість_розширення, '+
 'Server.Призначення_Назва AS Сервер '+
'FROM  '+
'Server, ' +
'Service '+
'WHERE '+
'Service.Сервер_ИД = Server.ID ';
...
' ConnectInfo.[Дата],'+
' Service.Назва,'+
' (Customers.[Прізвище] & " " + Customers.[Імя]) AS Клієнт,'+
' ConnectInfo.[ТермінДії],'+
' ConnectInfo.[Сервер_ID],'+
' ConnectInfo.[Ціна] '+
' FROM Customers, ConnectInfo INNER JOIN Service ON Service.ID = ConnectInfo.[Сервер_ID]'+
' WHERE'+
' Customers.ID = ConnectInfo.[Клієнт_ID] AND ConnectInfo.ID = '+Edit6.Text+' ORDER BY ConnectInfo.ID';
...
'Service '+
'WHERE '+
'(ConnectInfo.[Тариф_ID] = Service.ID AND '+
'ConnectInfo.[Клієнт_ID] = Customers.ID) '+
...
begin
DataModule2.ADOQService.Close;
DataModule2.ADOQService.SQL.Text:='INSERT INTO Service ( [Назва], [Тип], [Ціна], [Обсяг],[Можливість_розширення],[Сервер_ИД] ) values ("'+Edit1.Text+'","'+Edit2.Text+'","'+Edit3.Text+'","'+Edit4.Text+'",'+BoolToStr(CheckBox1.Checked)+','+IntToStr(DBLookupComboBox2.KeyValue)+')';
DataModule2.ADOQService.ExecSQL;

DataModule2.ADOQService.Close;
DataModule2.ADOQService.SQL.Text:= 'SELECT Service.ID, Service.Назва, Service.Тип, Service.Ціна, Service.Обсяг, Service.Можливість_розширення, Server.Призначення_Назва AS Сервер '
+'FROM Server, Service WHERE Service.Сервер_ИД = Server.ID ';
DataModule2.ADOQService.Open;
end;
end;
...
begin
     if ID <> ''  then
      ...
      DataModule2.ADOQService.SQL.Text:='UPDATE Service '+
          'SET '+
          'Service.Назва ="' +Edit1.Text+ '",'+
          'Service.Тип ="' +Edit2.Text+ '",'+
          'Service.Ціна ="' +Edit3.Text+ '",'+
          'Service.Обсяг ="' +Edit4.Text+ '",'+
          'Service.Можливість_розширення =' +BoolToStr(CheckBox1.Checked)+ ','+
          'Service.Сервер_ИД =' +IntToStr(DBLookupComboBox2.KeyValue)+ ' '+
          'WHERE '+
          'Service.ID =' + ID + ';';
      DataModule2.ADOQService.ExecSQL;
...
begin
    ...
    DataModule2.ADOQTest.SQL.Text:='SELECT ID, Призначення_Назва  FROM Server WHERE ( Призначення_Назва = "'+Server+'")';
...
DataModule2.ADOQConnnectInfo.SQL.Text:= 'SELECT '+
  ...
  'ConnectInfo.[Ціна] '+
  ' FROM Customers , ConnectInfo INNER JOIN Service ON Service.ID = ConnectInfo.[Сервер_ID] '+
  'WHERE Customers.[Прізвище] like "%'+Edit5.Text+'%" AND '+
  'Customers.ID = ConnectInfo.[Клієнт_ID]'+
  'ORDER BY ConnectInfo.ID';
DataModule2.ADOQConnnectInfo.Open;
end;
...
procedure TForm1.FormShow(Sender: TObject);
  begin
    ...
    'Service '+
    'WHERE '+
    'Service.Сервер_ИД = Server.ID ORDER BY Service.ID';
    DataModule2.ADOQService.Open;
  end;
